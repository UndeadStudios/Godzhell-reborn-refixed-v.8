...
public class stream {
    ...
    public byte[] decryptRSA(BigInteger d, BigInteger n) {
        ...
        for (int i = 0; i < rsaLength; i++) {
            rsaArray[i] = buffer[currentOffset++];
        }
    ...
    public long readQWord() {
        long l = (long) readInteger() & 0xffffffffL;
        long l1 = (long) readInteger() & 0xffffffffL;
        return (l << 32) + l1;
    }

    public long readQWord2() {
        currentOffset += 8;
        return (((long) (buffer[currentOffset - 8] & 0xff) << 56) + ((long) (buffer[currentOffset - 7] & 0xff) << 48) + ((long) (buffer[currentOffset - 6] & 0xff) << 40) + ((long) (buffer[currentOffset - 5] & 0xff) << 32) + ((long) (buffer[currentOffset - 4] & 0xff) << 24) + ((buffer[currentOffset - 3] & 0xff) << 16) + ((buffer[currentOffset - 2] & 0xff) << 8) + (buffer[currentOffset - 1] & 0xff));
    }
